import { parseCSV, generateCertificateId, type CSVRow } from '@/lib/utils/csvParser';
...
// Full CSV data - All 70 participants from ctrl_think_rows.csv
...
function csvToCertificates(csvRows: CSVRow[]): CertificateData[] {
  return csvRows.map(row => ({
    id: row.id,
    certificateId: generateCertificateId(row.id),
...
export const ctrlThinkParticipants: CertificateData[] = csvToCertificates(parsedRows);

// Helper functions
export function findCertificate(certificateId?: string, enrollment?: string): CertificateData | null {
  if (certificateId) {
    return ctrlThinkParticipants.find(cert => 
      cert.certificateId.toLowerCase() === certificateId.toLowerCase()
    ) || null;
  }
  if (enrollment) {
    return ctrlThinkParticipants.find(cert => 
      cert.enrollment.toLowerCase() === enrollment.toLowerCase()
    ) || null;
  }
  return null;
}
...
export function getAllCertificates(): CertificateData[] {
  return ctrlThinkParticipants;
}

export function getCertificatesByTeam(teamName: string): CertificateData[] {
  return ctrlThinkParticipants.filter(cert => 
